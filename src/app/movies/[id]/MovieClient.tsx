  ...
  return (
    <div className="min-h-screen bg-base-200">
      ...
      <main className="w-[97%] max-w-screen-xl min-h-fit lg:h-[85vh] mx-auto p-6 flex flex-col lg:flex-row gap-6 bg-cyan-50 text-black rounded-lg shadow-lg overflow-hidden">
        ...
        <section className="lg:w-1/2 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {genres.map((genre: any) => (
              <span key={genre} className="badge badge-primary">{genre.trim()}</span>
            ))}
          </div>
          <hr />
        ...
        {/* Reviews */}
        <section className="lg:w-1/2 space-y-4 overflow-y-scroll">
          ...
          <div className="flex flex-col lg:flex-row justify-center items-center gap-4 w-full">
            <div className="flex flex-col items-center text-center">
              ...
              <div className="text-sm text-gray-700">
                <p><strong>{total}</strong> reviews</p>
                <p>Avg: <strong>{avg.toFixed(1)}</strong> / 5</p>
              </div>
            </div>
          ...
          <div className="flex gap-2">
            <button className="btn btn-accent" onClick={() => setShowForm(true)}>Add Review</button>
                ...
                <div key={i} className="flex gap-2 p-3 bg-orange-100 shadow rounded-lg">
                  <div className="w-[20%] text-center border-r pr-2 flex flex-col items-center gap-2">
                    <img
                      ...
                      alt="profile pic"
                    />
                    <p className="font-bold text-sm break-words">{item.usr}</p>
                  </div>
                  <div className='w-[80%]'>

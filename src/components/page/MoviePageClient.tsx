...
export default function MoviePageClient({ movie }: { movie: Movie & { credits?: CreditsData } }) {
  ...
  return (
    <main className="text-on-surface pt-16 md:pt-20 min-h-screen flex flex-col md:flex-row">
        {/* Left Column: Content */}
        <div className="w-full md:w-1/3 pb-10 px-2 md:px-6 overflow-y-auto space-y-20 border-r border-white/20 no-scrollbar">
          ...
          <section className="space-y-12">
            ...
            >
              ...
              <div className="space-y-4 pt-2">
                <h1 className="text-5xl md:text-5xl font-headline font-bold leading-none tracking-tighter">{movie.title}</h1>
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2 font-label text-sm uppercase tracking-[0.2em] text-neutral-500">
                  <span className="flex gap-2">
                    {movie.genres?.map((g) => (
                      <span key={g}>
                        <Link href={`/genre/${g}`} className="hover:text-white transition-colors uppercase">
                          {g}
                        </Link>
                      </span>
                    ))}
              ...
              <div className="flex overflow-x-auto gap-6 no-scrollbar pb-4">
                {movie.credits.actors.map((actor) => (
                  <div key={actor.id} className="min-w-25 max-w-25 space-y-4">
                    <div className="w-full aspect-square bg-surface-container-high grayscale hover:grayscale-0 transition-all duration-500 overflow-hidden rounded-md">
                      <img 
                        className="w-full h-full object-cover" 
                        src={actor.profile_image ? actor.profile_image : "/images/avatar.webp"} 
                        alt={actor.name}
                        loading="lazy"
                        decoding="async"
                      />
        ...
        <div className="hidden md:block w-2/3 h-[calc(100vh-6rem)] sticky top-20 overflow-hidden">
